...
const distanceSq = (dimensions) =>
	Math.abs(dimensions.map((d) => d * d).reduce(reduceSum, 0));
const isDefined = (v) => Boolean(v);
const interlaceArrays = (a, b) =>
	a.flatMap((v, i) => [v, b[i]]).filter(isDefined);
...
const isWithinBounds = ([x, y], [x1, y1], [x2, y2]) =>
	x >= x1 && y >= y1 && x < x2 && y < y2;
export const filterWithinBounds = (a, b) => (pos) =>
...
const defineUnwritableProperty = (obj, prop, value, attributes = {}) =>
...
const rotateNormBy90Deg = (curDir, turns) =>
	rotateNormBy45Deg(curDir, 2 * turns);
